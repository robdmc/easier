...
class Elliptic:
    ...
    def _check_freqs(self, f_pass, f_stop, f_sample):
    ...
    def _get_filter_coeffs(self, output='sos', analog=False):
        N, Wn = signal.ellipord(wp=self._f_pass, ws=self._f_stop, gpass=self._max_suppression_pass, gstop=self._min_suppression_stop, analog=False, fs=self._f_sample)
        if analog:
            fs = None
        else:
            fs = self._f_sample
        coeffs = signal.ellip(N, rp=self._max_suppression_pass, rs=self._min_suppression_stop, Wn=Wn, btype=self._kind, analog=analog, output=output, fs=fs)
        return coeffs
    ...
    def plot_response(self, n_points=5000):
        """
    ...
    def plot_noise_response(self, n_points=5000):
        """
