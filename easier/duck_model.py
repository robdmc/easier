...
class Table:
    ...
    def create(self, df):
        self.drop()
        self.query(f'BEGIN TRANSACTION; CREATE TABLE {self.table_name} AS SELECT * FROM __df_in__; COMMIT', fetch=False, __df_in__=df)
        self.exists_dict = {}
        self.duck.connection.unregister('__df_in__')

    def insert(self, df):
        self.ensure_writeable()
        self.query(f'BEGIN TRANSACTION INSERT INTO {self.table_name} SELECT * FROM __df_in__ COMMIT', fetch=False, __df_in__=df)
        self.duck.connection.unregister('__df_in__')
...
class DuckModel:
    ...
    def __dir__(self):
        return ['tables', 'table_names', 'query', 'explain', 'reset_connection', 'set_index', 'list_indexes', 'file_name']
