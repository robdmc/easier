...
class MiniModelBase:
    ...
    def __init__(self, file_name_or_url='', overwrite=False, read_only=False):
        raise NotImplementedError('You must write your own constructor that creates a connector and allows for overwrite=True/False and read_only=True/False')
...
class MiniModelPG(MiniModelBase):
    ...
    def __getattr__(self, name):
        mapper = {'host': 'PGHOST', 'user': 'PGUSER', 'password': 'PGPASSWORD', 'database': 'PGDATABASE', 'port': 'PGPORT'}
        if name in mapper:
            try:
